...
MB_Status_t MB_Init_UART1(UART_HandleTypeDef *huart, uint8_t SLA) {
	...
	MB.InputBitsSize = 1;    // 2 bytes for input bits (16 bits)
	MB.CoilBitsSize = 1;     // 2 bytes for coil bits (16 bits)
	...
	MB.TX_Automplete = 0; // Disable auto-complete for TX (manual control)
	...
	MB_Status_t S = MB_Slave_Init(&MB);
